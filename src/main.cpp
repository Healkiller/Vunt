...
#if defined(NDEBUG)
# error "Vunt cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "Vunt Signed Message:\n";
...
int64_t GetProofOfStakeReward(int64_t nCoinAge, int64_t nFees)
{
    int64_t nSubsidy = nCoinAge * COIN_YEAR_REWARD * 33 / (365 * 33 + 8);
                         ...
                         {
                                 nSubsidy = nSubsidy * 2 ;  //300%
                         }

                            else if(nBestHeight <= 75000)
                         {
                                 nSubsidy = nSubsidy * 10 ;  //1000% Superblocks
                         }
